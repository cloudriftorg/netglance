...
The `netglance.conf` is generated by CI and published alongside the
pkg metadata, so this single command is shell-agnostic (works in
sh, csh, tcsh, bash) and avoids heredoc quoting quirks. If you'd
rather drop the file in by hand, the contents are:
