...
The published repo is signed with an RSA pubkey. Setup is two fetches —
the trusted public key and the conf that points pkg(8) at it.
...
The `netglance.conf` is generated by CI and references the pubkey path
above; you don't have to hand-craft it. After the two fetches the file
should look like:
...
```sh
openssl rsa -in /usr/local/etc/pkg/keys/netglance.pub -pubin -outform DER 2>/dev/null \
  | sha256sum
```
