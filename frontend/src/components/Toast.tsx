...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastCtx.Provider value={value}>
      {children}
      <div className="pointer-events-none fixed inset-x-0 top-0 z-50 flex flex-col items-center gap-2 px-3 pt-3">
        {items.map((t) => (
          <div
            key={t.id}
            role="status"
            ...
            className={clsx(
              ...
              t.kind === 'success' && 'bg-emerald-600 text-white ring-emerald-700/20',
              t.kind === 'error' && 'bg-red-600 text-white ring-red-700/20',
              t.kind === 'info' && 'bg-sky-600 text-white ring-sky-700/20'
            )}
          >
